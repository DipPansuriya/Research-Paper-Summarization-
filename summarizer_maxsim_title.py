# -*- coding: utf-8 -*-
"""
Created on Thu Dec  1 23:12:54 2016
...
def summarize_maxsim(filename):
    ...
    title_dict = {}
    for i in range(1, tfidf_matrix.shape[0]):
        title_dict[i] = sim_scores[0][i]
    
    #combine similarity with title and other sentences    
    final_dict = {}
    for key in scores_dict:
        final_dict[key] = (0.025 * scores_dict[key]) + (0.975 * title_dict[key])
    sorted_final = sorted(final_dict.items(), key=operator.itemgetter(1), reverse=True)
    ...
    for i in range(0, limit):
        summary = summary + (docs[sorted_final[i][0]] + ". ")
    #create output file to write the summary
    out_filename = "C:/USC stuff/Applied NLP/Group Project/Test1/Sys2/abstract" + os.path.basename(filename)
    print(out_filename)
